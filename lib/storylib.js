...
const $story = {
    ...
    init: () => {
        ...
        if ($story.url.hostname == "localhost") {
            const body = $("body")[0];
            body.innerHTML = `
            <div style="background-color:#f4a43a70;color:#543308;position:absolute;top:0px;left:0px;right:0px;width:100%;margin:0px;height:fit-content;z-index:999999;text-align:center;padding:10px;" onclick="this.remove();"><b>BROWSER LINAKGE WARNING!</b> StoryLib network communications are redirected to <b>localhost:5501</b></div>
            ` + body.innerHTML;
        }
    },
    sections: {},
    ...
    wiki: {
        ...
        fetchAndRender: (loading_screen, content_form) => {
            $story.network.text(
                ...
                md_ => {
                    ...
                    const btn = $("#brandButton")[0];
                    if ($story.wiki.data.file.startsWith("wiki/")) {
                        btn.innerHTML += " <span>WIKI</span>";
                    } else if ($story.wiki.data.file.startsWith("blog/")) {
        ...
        renderCogwheelSpec: (loading_screen, content_form, json) => {
            ...
            let html = "";
            if (json.type == "OBJECT") {
                html += `<div class="cw-global"><div class="cw-top"><h2 class="cw-title">${json.name}</h2><p class="cw-summary">${md(json.summary)}</p></div><table class="cw-table"><thead><tr><th>Method</th><th>Description</th></tr></thead>`;
                const methods = Object.keys(json.methods);
                $story.data = {};
                $story.data.alerts = {};
                if (json.hasOwnProperty("parent")) {
                    let parentsHTML = "";
                    let parents = json.parent.split(",");
                    for (let i = 0; i < parents.length; i++) {
                        if (i != 0) {
                            parentsHTML += " and ";
                        }
                        parentsHTML += `<a onclick="$story.to('/wiki/wiki.html?p=wiki/projects/cogwheel/specs/${parents[i]}.sa.json')">${parents[i]}</a>`;
                    }
                    html += `<tr><th>${json.name} does have all methods of ${parentsHTML}</th></tr>`;
                }
                for (let i = 0; i < methods.length; i++) {
                    ...
                    if (returnType != "null") {
                        if (returnType === "*") {
                            returnType = `<span>unknown type. (Return type calculated dynamicly)</span>`;
                        } else {
                ...
                html += `</table><div class="cw-table" style="display:flex;flex-direction:column;"><div class="cw-th">This specification was last changed for version <span class="badge b-cf cw-badge cw-version-block">${json.version}</span></div><div class="cw-th">You can report problem <a onclick="$story.to('/bugreporting.html?id=wiki')">here</a></div></div>`;
                const btn = $("#brandButton")[0];
                btn.innerHTML += " <span>SPECIFICATIONS</span>";
                $story.network.text(
                    $story.wiki.data.offline
                    ? `https://raw.githubusercontent.com/StoryAnvil/${$story.wiki.data.repo}/${$story.wiki.data.branch}/wiki/projects/cogwheel/specs/list.json`
                    : `https://raw.githubusercontent.com/wiki/projects/cogwheel/specs/list.json`,
                    !$story.wiki.data.offline,
                    list => {
                        const ar = JSON.parse(list);
                        const btns = $("#brandButtons")[0];
                        let options = "<option value=\"-\" disabled selected>Go to spec</option>";
                        for (let opt = 0; opt < ar.length; opt++) {
                            options += `<option value="${ar[opt]}">${ar[opt]}</option>`
                        }
                        btns.innerHTML += "<select id=\"specSelect\" onchange=\"$story.to('/wiki/wiki.html?p=wiki/projects/cogwheel/specs/' + this.value + '.sa.json')\">" + options + "</select>";
                    },
                    console.error, 5501
                )
                html += "</div>"
